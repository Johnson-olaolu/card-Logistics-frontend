	...
	async function getCompanies(query) {
		...
		if (response.success) {
			...
			setUnCompletedClusterManagersLgas(
				...
				)[0].lgas.filter(lga => (response?.data?.clusterManagers.filter(m => m.localGovernment == lga && m.isAccepted).length == 0))
			);

			setUnCompletedLogiticsCompanyLgas(
				NigerianStates.filter(
					(state) => state.state == query
				)[0].lgas.filter(lga => (response?.data?.logisticCompanies.filter(m => m.localGovernment == lga && m.isAccepted).length == 0))
			);
		} else {
			//show custom modal
		...
		<>
				...
				<div onClick={() => navigate("/state-coordinator/home")}  className=" absolute left-8 top-8 text-gray-500 cursor-pointer">
					...
					<div className="">
						...
						<div className=" flex justify-between items-center mt-8">
							...
							<ConfirmationAlert completed={unCompletedClusterManagersLgas.length === 0 && unCompletedLogiticsCompanyLgas.length === 0} onClick={() => {setShowConfirmationErrorModal(true)}} />
						</div>
					</div>
				</div>
			...
			{showConfirmationErrorModal && (
				<ConfirmationErrorModal 
					unCompletedClusterManagersLgas = {unCompletedClusterManagersLgas}
					unCompletedLogiticsCompanyLgas = {unCompletedLogiticsCompanyLgas}
